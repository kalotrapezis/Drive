...
## Connection cards in the development build

Settings → Connection management supports Copy and verified Move, including last-day/week/month/year retention. Preview and transfer directly from each card. Originals remain until **Review source cleanup → Confirm move to Trash**; both copies are checked again before cleanup. Retention uses modification time and runs only when confirmed. Reopen Preview route to recover pending cleanup after closing the page or restarting. Changed settings require a fresh preview; active routes cannot be edited or removed. Cache intake and reviewed forwarding are available; reverse/two-way sync and unpairing remain unavailable.
...
dpkg-deb --info build-release/packages/local-drive_0.1.0~alpha3_amd64.deb
```
...
Device management keeps Visible and Hidden devices together: hiding selects the Hidden list, and Show again restores dashboard visibility. Remove device stops its saved connections and removes its app listing while retaining files and verified history. This local catalog removal does not revoke wireless credentials.

Year retention uses one calendar year before review, based on last modification time. Catalog schema 18 supports this policy; use the updated development binaries with an upgraded catalog (older binaries reject schema 18).

## Scheduled transfers

Settings → Sync scheduling supports one-off dates and daily, weekly or monthly runs in the saved time zone. Monthly dates clamp to the end of shorter months and return to the originally chosen day afterward. Local Drive's backend must remain running; the browser may be closed. The scheduler does not wake a sleeping laptop or start at login. Late runs, unavailable devices, conflicts and interrupted jobs remain pending until Start is pressed. Configuration is saved atomically beside the catalog in `.schedules.json`.

Select Laptop library for an existing computer-to-disk connection, or a known USB/MTP phone to import Drive/DCIM into that connection's laptop source first. The import retains phone originals, respects the connection’s cache disk percentage and uses the existing bounded intake/verification engine. If the final disk is absent, the verified laptop copy remains pending for explicit forwarding. Move transfers still require Review source cleanup in the connection card. Desktop tray notices and persistent dashboard notices report pending and completed work.

Connection Cache is configured on each card, with a disk fullness percentage in Advanced (80% default, 1–95%). Limits persist across disable/re-enable. USB phone intake uses the connection source library while the destination is absent, preserving phone originals; the usual verified route forwards those files when the user starts it. Scheduled phone intake requires Cache enabled. The free-space reserve is enforced before each file; notices check disk capacity every 30 seconds and also report import failures. Old separate staging folders are retained and must be reviewed before re-saving the cache configuration. Cache limits are local to this computer and connection; remote hosts configure their own local disks.
