...
export type GalleryPhoto = { path: string; name: string; size: number; modified: string; captured: string; dateSource: string; type: "Photo" | "Video"; collection: string; demoCrop?: string; source?: "Photos" | "Screenshots" | "Archive"; archive?: { storageId: string; name: string } };
export const mediaKey = (item: GalleryPhoto) => item.archive ? `Archive/${item.archive.storageId}/${item.archive.name}/${item.path}` : `${item.source || "Photos"}/${item.path}`;
export const mediaUrl = (item: GalleryPhoto, preview = false) => item.archive ? `/api/v1/archive-media?storageId=${encodeURIComponent(item.archive.storageId)}&name=${encodeURIComponent(item.archive.name)}&path=${encodeURIComponent(item.path)}${item.type === "Video" ? "&raw=1" : preview ? "&preview=1" : ""}` : `/api/v1/photo-thumbnail?root=${item.source || "Photos"}&path=${encodeURIComponent(item.path)}${preview ? "&preview=1" : ""}`;

export function PhotoViewer({ items, selected, onSelect, onClose, fixture, imageStyle, imageEditor }: { items: GalleryPhoto[]; selected: GalleryPhoto; onSelect: (item: GalleryPhoto) => void; onClose: () => void; fixture: boolean; imageStyle: (item: GalleryPhoto) => CSSProperties | undefined; imageEditor: boolean }) {
  ...
  useEffect(() => { if (!selected.archive) return; const index = items.findIndex((item) => mediaKey(item) === mediaKey(selected)); items.slice(index + 1, index + 3).filter((item) => item.type === "Photo").forEach((item) => { const image = new Image(); image.src = mediaUrl(item, true); }); }, [items, selected]);
  ...
  const showMap = async () => {
    ...
    if (selected.archive) { setMapStatus("Archive entries are read-only; location is not loaded until you extract a copy."); return; }
  ...
  return <dialog ref={dialog} className="media-viewer" aria-label="Photo viewer" onClose={onClose} onCancel={() => setPlaying(false)} onKeyDown={(event) => { if ((event.target as HTMLElement).tagName === "INPUT") return; if (event.key === "ArrowRight") { event.preventDefault(); move(1); } if (event.key === "ArrowLeft") { event.preventDefault(); move(-1); } }}>
    ...
    <div className="media-focus-layout"><div className="media-focus-main"><div className="media-viewer-body">
      ...
      <button disabled={!document.fullscreenEnabled} onClick={() => void fullScreen()} title={document.fullscreenEnabled ? "Fullscreen" : "Fullscreen is unavailable in this browser window"} aria-label="Fullscreen"><ArrowsOut /><span>Fullscreen</span></button>
      ...
      <button disabled={selected.type === "Video" || !!selected.archive} onClick={() => void desktopAction("reveal-photo")} title={selected.archive ? "Extract a copy before opening it in a folder" : "Open containing folder"}><FolderOpen /><span>Open in folder</span></button>
      <button disabled={selected.type === "Video" || !!selected.archive || !imageEditor} onClick={() => void desktopAction("edit-photo")} title={selected.archive ? "Archive entries are read-only; extract a copy before editing" : imageEditor ? "Edit in desktop image editor" : "No supported image editor detected, or local service unavailable"}><PencilSimple /><span>Edit</span></button>
